...
utter(holds(Atom), Options) -->
    { css_classes(Options, Classes) },
    (   { atom(Atom) }
    ->  emit([span(class(Classes), Atom), ' tiene'])
    ;   { Atom =.. [Name|Args] }
    ->  emit([span(class(Classes), Name), ' tiene per ']),
        list(Args, Options)
    ).
...
scasp_term(@(Value:''), Options) -->
    !,
    scasp_term(Value, Options).
% Uncomment to enable the type printing in human output
...
connector(implies, _Options) -->
    {pengine_self(M), M:source_lang(es)}, 
    emit([ span(class(human), ', porque'),
           span(class(machine), ' \u2190')
         ]).

connector(and, _Options) -->
    {pengine_self(M), M:source_lang(it)},
    emit([ span(class(human), ', e'),
           span(class(machine), ',')
         ]).
connector(not, _Options) -->
    {pengine_self(M), M:source_lang(it)},
    emit([ span(class(human), 'non è provato che '),
           span(class(machine), 'not ')
         ]).
connector(-, _Options) -->
    {pengine_self(M), M:source_lang(it)},
    emit([ span(class(human), 'non è il caso che '),
           span(class(machine), '\u00ac ')
         ]).
connector(implies, _Options) -->
    {pengine_self(M), M:source_lang(it)},
    emit([ span(class(human), ', perché'),
           span(class(machine), ' \u2190')
         ]).
